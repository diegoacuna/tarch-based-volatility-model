...
garchKappa <-  
    ...
    "snig", "pearsonIV"), gamma = 0, delta = 2, skew = NA, shape = NA)
{   
...
.garchKappaFun <-  
    function(x, 
    cond.dist = c("norm", "ged", "std", "snorm", "sged", "sstd", "snig", "pearsonIV"), 
...
{   
    ...
    if (cond.dist == "pearsonIV") {
        fun = funcE * dpearsonIV(x, nu = skew, m = shape)
    }
